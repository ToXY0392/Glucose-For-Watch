...
/**
 * Extrait IP, ports et code de jumelage à partir du texte brut renvoyé par la reconnaissance OCR
 * (écrans « débogage Wi‑Fi » / jumelage sur la montre).
 */
...
object WearInstallOcrParser {
    /**
     * Corrige des erreurs fréquentes de ML Kit (O/o pour 0 dans une IP ou un port).
     */
    ...
    /** Tolère les espaces insérés par l’OCR : « 192 . 168 . 1 . 23 ». */
    private val ipv4Loose =
        Regex(
            """\b(?:\d{1,3}\s*\.\s*){3}\d{1,3}\b""",
        )
    private val sixDigitCode = Regex("""(?<![0-9])(\d{6})(?![0-9])""")
    private val sixDigitCodeSpaced = Regex("""(?<![0-9])((?:\d\s*){6})(?![0-9])""")
    /** Port seul sous l’IP (écran « jumeler » sur Wear). */
    private val standaloneFiveDigitLine = Regex("""^\s*(\d{4,5})\s*$""")
    /**
     * Ligne d’en-tête type « IP address & port » / débogage sans USB (écran principal).
     * Évite « pair » / « pairing » pour ne pas confondre avec l’écran de jumelage.
     */
    private val adbContextHints =
        Regex(
            """(?i)(ip\s+address\s*(\u0026|and|\+)?\s*port|address\s*&\s*port|(?:wireless|wifi).*debug|d(e|é)bogage[^\n]{0,12}wi)""",
        )
    private val ipColonPort =
        Regex(
            """(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\s*[:#]\s*(\d{4,5})\b""",
            RegexOption.IGNORE_CASE,
        )
    private val ipColonPortLoose =
        Regex(
            """(?:\d{1,3}\s*\.\s*){3}\d{1,3}\s*[:#]\s*(\d{4,5})\b""",
            RegexOption.IGNORE_CASE,
        )
    ...
    fun parse(raw: String): WearInstallOcrParsed {
        ...
        val colonPorts =
            buildList {
                addAll(
                    ipColonPort.findAll(text).mapNotNull { m ->
                        m.groupValues.getOrNull(1)?.toIntOrNull()?.takeIf { it in PORT_RANGE }
                    },
                )
                if (isEmpty()) {
                    addAll(
                        ipColonPortLoose.findAll(text).mapNotNull { m ->
                            m.groupValues.getOrNull(1)?.toIntOrNull()?.takeIf { it in PORT_RANGE }
                        },
                    )
                }
        ...
        for (m in portToken.findAll(text)) {
            ...
            if (n !in PORT_RANGE) continue
            standalonePorts.add(n)
        }
        ...
        val portsForPair =
            orderedDistinctPorts.filter { adbPort == null || it != adbPort }.toMutableList()

        when {
            adbPort != null -> {
                if (pairPort == null) pairPort = portsForPair.firstOrNull { it != adbPort }
            }
            orderedDistinctPorts.size >= 2 -> {
                if (pairPort == null) pairPort = orderedDistinctPorts[0]
                adbPort = orderedDistinctPorts[1]
            }
            orderedDistinctPorts.size == 1 -> {
                if (pairPort == null) pairPort = orderedDistinctPorts[0]
            }
        }
    ...
    private fun inferPairPortBetweenIpAndCode(
        lines: List<String>,
        ip: String?,
        pairCode: String?,
    ): Int? {
        ...
        val codeLineIdx =
            if (pairCode != null) {
                lines.indexOfFirst { Regex("""\b${Regex.escape(pairCode)}\b""").containsMatchIn(it) }
                    .takeIf { it >= 0 }
            } else {
    ...
    private fun inferAdbPortFromKeywordLines(lines: List<String>, ip: String?): Int? {
        ...
        for (i in lines.indices) {
            ...
            val window =
                sequenceOf(lines.getOrNull(i), lines.getOrNull(i + 1)).filterNotNull().joinToString(" ")
        ...
        if (ip != null) {
            ...
            if (ipIdx >= 0) {
                for (delta in 0..2) {
                    ...
                    val concat =
                        "${lines[j]} ${lines[j + 1]}".replace(Regex("""\s+\.\s+"""), ".")
    ...
    private fun extractPortFromIpColon(blob: String): Int? {
        val m =
            ipColonPort.find(blob) ?: ipColonPortLoose.find(blob) ?: return null
    ...
    private fun normalizeOcrText(raw: String): String {
        ...
        // IP : rapprocher chiffres et points (« 192 . 168 » → « 192.168 »)
        var prev: String
        do {
            prev = t
            t = t.replace(Regex("""(\d)\s*\.\s*(\d)"""), "$1.$2")
        } while (t != prev)
        // Ports / codes découpés : « 37 261 », « 123 456 »
        t = t.replace(Regex("""\b(\d{2})\s+(\d{3})\b"""), "$1$2")
        t = t.replace(Regex("""\b(\d{3})\s+(\d{2})\b"""), "$1$2")
        t = t.replace(Regex("""\b(\d{3})\s+(\d{3})\b"""), "$1$2")
        // Dernier filet : espaces entre chiffres contigus (hors cas déjà traités)
