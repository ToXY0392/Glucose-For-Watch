...
if [[ -z "$PHONE_SERIAL" || -z "$WATCH_SERIAL" ]]; then
  echo "[widget-g7] Missing serials. Set WIDGETG7_PHONE_SERIAL and WIDGETG7_WATCH_SERIAL." >&2
  exit 1
fi
...
echo "[widget-g7] Collecting diagnostics into $OUT_DIR"
...
echo "[widget-g7] Diagnostics collection complete"
echo "$OUT_DIR"
